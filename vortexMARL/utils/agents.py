...
from utils.vortex_policies import DiscretePolicy

class AttentionAgent(object):
    """
    General class for Attention agents (policy, target policy)
    """
    def __init__(self, sa_size, hidden_dim=64,
        ...
        """
        self.policy = DiscretePolicy(sa_size,
                                     hidden_dim=hidden_dim,
                                     onehot_dim=onehot_dim)
        self.target_policy = DiscretePolicy(sa_size,
                                            hidden_dim=hidden_dim,
                                            onehot_dim=onehot_dim)
